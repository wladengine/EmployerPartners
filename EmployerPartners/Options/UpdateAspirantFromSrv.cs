...
{
    public partial class UpdateAspirantFromSrv : Form
    {
        ...
        private void GetDateUpdated()
        {
            try
            {
                ...
                {
                    var x = context.AspirantData.First();
        ...
        private void FillCombo()
        {
            ComboServ.FillCombo(cbDegreeName, HelpClass.GetComboListByQuery(@" select distinct [DegreeName]  AS Id, 
                                    [DegreeName] as Name from dbo.AspirantData order by [DegreeName]"), false, true);
            ComboServ.FillCombo(cbCourse, HelpClass.GetComboListByQuery(@" select distinct CONVERT(varchar(100), [Course]) AS Id, 
                                    CONVERT(varchar(100), [Course]) as Name from dbo.AspirantData order by [Name]"), false, true);
            ComboServ.FillCombo(cbDepartment, HelpClass.GetComboListByQuery(@" select distinct [StudyForm]  AS Id, 
                                    [StudyForm] as Name from dbo.AspirantData order by [StudyForm]"), false, true);
            ComboServ.FillCombo(cbStatusName, HelpClass.GetComboListByQuery(@" select distinct [StatusName]  AS Id, 
                                    [StatusName] as Name from dbo.AspirantData order by [StatusName] desc"), false, true);

            ComboServ.FillCombo(cbFacultyName, HelpClass.GetComboListByQuery(@" select distinct [FacultyName]  AS Id, 
                                    [FacultyName] as Name from dbo.AspirantData order by [FacultyName]"), false, true);
            ComboServ.FillCombo(cbSpecName, HelpClass.GetComboListByQuery(@" select distinct [SpecName]  AS Id, 
                                    [SpecName] as Name from dbo.AspirantData order by [SpecName]"), false, true);
            ComboServ.FillCombo(cbObrazProgram, HelpClass.GetComboListByQuery(@" select CONVERT(varchar(100), [Id]) AS Id, ([Number] + '  ' + Name) as Name  
                                    from dbo.ObrazProgram where Id in (select ObrazProgramId from dbo.AspirantData) order by dbo.ObrazProgram.Name"), false, true);
        }
        private void FillStudent()
        {
            try
            {
                using (EmployerPartnersEntities context = new EmployerPartnersEntities())
                {
                    var lst = (from x in context.AspirantData
                               ...
                               where ((!String.IsNullOrEmpty(DegreeName)) ? x.DegreeName == DegreeName : true) &&
                                        ((Course.HasValue) ? x.Course == Course : true) &&
        ...
        private void SetFilter()
        {
            ...
            strFilter += ((strFilter == "") ? "" : " AND ") + ((Course.HasValue) ? ("Курс = '" + Course + "'") : "");
        ...
        private void btnUpdateStudent_Click(object sender, EventArgs e)
        {
            ...
            pBar1.Value = 60;
            pBar1.Invalidate();
            if (Utilities.UpdateAspirantData())
            {
                //this.UseWaitCursor = false;
                pBar1.Value = 90;
